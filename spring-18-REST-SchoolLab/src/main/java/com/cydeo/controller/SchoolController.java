...
public class SchoolController {
    ...
    public ResponseEntity<ResponseWrapper> readAllStudents(){
        ...
        //if we use ResponseEntity.ok, then we put the data(body) directly as above
    }

    @GetMapping("/parents")
    public ResponseEntity<ResponseWrapper> readAllParents(){
        ResponseWrapper responseWrapper = new ResponseWrapper(true, "Parents are retrieved succesfully",
                HttpStatus.OK.value(), parentService.findAll());

        return ResponseEntity.status(HttpStatus.OK).body(responseWrapper);
        //if we use ResponseEntity.status(HttpStatus.OK), then we need to use .body as above
    }
